...
from dataset.gi4e_max import gi4e_eye
import matplotlib.pyplot as plt
import time

parser = argparse.ArgumentParser()
parser.add_argument('--data_path', default=r'/home/snowwhite/eye_tracking/gi4e_database/blend')
parser.add_argument('--train_path', default=r'/home/snowwhite/eye_tracking/gi4e_database/5fold-new-max/data_train_5.json')
parser.add_argument('--val_path', default=r'/home/snowwhite/eye_tracking/gi4e_database/5fold-new-max/data_test_5.json')
parser.add_argument('--test_path', default=r'/home/snowwhite/eye_tracking/gi4e_database/5fold-new-max/data_test_5.json')
...
parser.add_argument('--sigma', default=0.5)               # 可选[0.5, 1.0, 2.0]
...
def train(model, criterion, optimizer, scheduler, dataTrain, dataVal):
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{epochs}") as bar:
            for i, (img, heatmap, eye_pts, pupil_dist, Width, Height, LorR, img_id) in enumerate(train_loader):
                img = img.cuda()
                ...
                error, norm_error, all_norm_error = cal_batch_error(pred_hm, eye_pts, Width, Height, pupil_dist, int(args.randround))
        ...
        if save_cp_and_plot:
            checkpoints_dir = f"results-max/{args.model}_sz{args.img_size}_sig{args.sigma}_ep{args.epochs}_{args.loss}_r{args.randround}"
    ...
    mean_test_loss, mean_test_error, mean_inference_time, mean_inference_time_per_frame, error_dict = test_main(model, os.path.join(checkpoints_dir, 'best.pth'))
    #print(error_dict)
    output_dict = {
...
def validate(model, val_loader, criterion):
    if len(val_loader) == 0:
        ...
        with tqdm(total=len(val_loader), desc="Validation") as vbar:
            for i, (img, heatmap, eye_pts, pupil_dist, Width, Height, LorR, img_id) in enumerate(val_loader):
                img = img.cuda()
                heatmap = heatmap.cuda()
                eye_pts =eye_pts.cuda()
                pupil_dist = pupil_dist.cuda()
...
def test_main(model, weight_path):
    ...
    mean_test_loss, mean_test_error, mean_inference_time, mean_inference_time_per_frame, error_dict = test(model, test_loader, criterion)
    return mean_test_loss, mean_test_error, mean_inference_time, mean_inference_time_per_frame, error_dict
...
def test(model, test_loader, criterion):
    if len(test_loader) == 0:
        ...
        with tqdm(total=len(test_loader), desc="Test") as vbar:
            for i, (img, heatmap, eye_pts, pupil_dist, Width, Height, LorR, img_id) in enumerate(test_loader):
                ...
                for i, batch_errors in enumerate(all_norm_error.detach().cpu().numpy()):
                    #for e in batch_errors:
                    e = batch_errors[1]
                    current_id = img_id[i]
                    current_LorR = LorR[i]
                    if current_id not in error_dict:
                        error_dict[current_id] = []
                    error_dict[current_id].append(e)
                    total_images += 1
                vbar.set_postfix(**{'loss per batch': loss.item()})
                vbar.update()
            error_rate_0025, error_rate_005, error_rate_001 = cal_error_max(error_dict)
        ...
        print("Error < 0.025:", error_rate_0025, "Error < 0.05:", error_rate_005, "Error < 0.01:", error_rate_001)
...
def cal_error_max(error_dict):
    error_count_0025 = 0
    error_count_005 = 0
    error_count_001 = 0
    total_images = len(error_dict)
    for value in error_dict.values():
        ...
        if e < 0.1:
            error_count_001 += 1
    error_rate_0025 = error_count_0025 / total_images
    error_rate_005 = error_count_005 / total_images
    error_rate_001 = error_count_001 / total_images
    
    return error_rate_0025, error_rate_005, error_rate_001
